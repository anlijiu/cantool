//
// Created by anlijiu on 18-1-5.
//

#ifndef CAN_CANALYST_CANALYSTII_H
#define CAN_CANALYST_CANALYSTII_H
...
public:
  ...
  unsigned int
  receive_can_frame(unsigned int can_idx, VCI_CAN_OBJ *recv_obj, unsigned int recv_len, int wait_time = 0);

  // send CAN frame from CAN bus
  bool send_can_frame(unsigned int can_idx, PVCI_CAN_OBJ send_obj, unsigned int send_len);
...
#endif //CAN_CANALYST_CANALYSTII_H
