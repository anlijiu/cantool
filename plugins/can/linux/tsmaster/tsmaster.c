...
static int tsmaster_set_bittiming(struct can_device *dev, enum BAUDRATE baudrate) {
    ...
    tscan_config_can_by_baudrate(tdev->ADeviceHandle, CHN1, timing_table[dev->bittiming.bitrate], 1);
    tscan_config_canfd_by_baudrate(tdev->ADeviceHandle, CHN1, timing_table[dev->bittiming.bitrate], timing_table[dev->data_bittiming.bitrate], lfdtISOCAN, lfdmNormal,1);

    tscan_config_can_by_baudrate(tdev->ADeviceHandle, CHN2, timing_table[dev->bittiming.bitrate], 1);
    tscan_config_canfd_by_baudrate(tdev->ADeviceHandle, CHN2, timing_table[dev->bittiming.bitrate], timing_table[dev->data_bittiming.bitrate], lfdtISOCAN, lfdmNormal,1);
}
...
static bool tsmaster_sendfd(struct can_device * dev, canfd_frame_t *frames, unsigned int len) {
    ...
    uint32_t result = 0;
    for(int i = 0; i < len; ++i, p++) {
        result = tscan_transmit_canfd_async(tdev->ADeviceHandle, p);
    }
    free(pObj);
    return true;
}
